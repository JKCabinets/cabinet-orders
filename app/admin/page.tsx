  ...
  return (
    <div className="bg-[#111] border border-[rgba(255,255,255,0.10)] rounded-xl p-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <p className="text-xs font-medium text-[rgba(232,227,218,0.50)]">Change password for <span className="text-[#e8e3da]">{member.name}</span></p>
      </div>
      ...
      <div className="flex flex-col gap-3 mb-4">
        <div>
          ...
          <div className="relative">
            <input type={showNew ? "text" : "password"} value={newPassword} onChange={(e) => { setNewPassword(e.target.value); setError(""); }} placeholder="Min. 6 characters" autoFocus className="field-input pr-9" />
      ...
      <div className="flex gap-2">
        ...
        <button onClick={handleSave} disabled={!newPassword || !confirmPassword} className="flex-1 py-2 rounded-lg bg-[rgba(255,255,255,0.04)] border border-[rgba(86,100,72,0.55)] text-xs text-[#e8e3da] font-medium hover:bg-[rgba(255,255,255,0.06)] disabled:opacity-40 disabled:cursor-not-allowed transition-all">Update password</button>
      </div>
    </div>
  );
}
