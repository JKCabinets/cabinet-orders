...
import { Users, Plus, Pencil, UserX, Trash2, Shield, User, Check, X, KeyRound, Eye, EyeOff, UserCog, ChevronLeft } from "lucide-react";
...
/**
 * /admin/team - team member management (admin only).
 *
 * Split out of /admin on 2026-07-29 so the admin landing page can carry the
 * tool buttons and, later, the sales-metrics panel. Everything that mutates
 * team_members lives here: add, edit, profile, password, deactivate, delete.
 *
 * Not to be confused with /team, which is the read-only staff directory
 * (avatars, contact details, online/OOO status) visible to any signed-in user.
 *
 * SECURITY: the isAdmin check below hides UI. It does NOT protect data.
 * The real gate is route-level - requireAdmin() on POST /api/team and
 * DELETE /api/team/[id], requireSelfOrAdmin() on PATCH /api/team/[id].
 * Same pattern as /admin and /admin/mappings.
 */
