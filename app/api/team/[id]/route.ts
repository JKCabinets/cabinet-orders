...
export async function PATCH(
  ...
  if (body.password) {
    ...
    if (pwd.length < 6) return NextResponse.json({ error: "Password must be at least 6 characters" }, { status: 422 });
    updates.password_hash = await bcrypt.hash(pwd, 12);
    updates.password = pwd; // keep plain text during migration, remove later
